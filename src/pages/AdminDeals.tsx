...
const AdminDeals = () => {
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(
      ...
      (err) => console.error('Deals error:', err)
    )
    return () => unsubscribe()
  }, [])
  ...
  const resetForm = () => {
    setFormData({ name: '', description: '', price: 0, image: '', items: [] })
    setItemsText('')
    setEditingDeal(null)
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.name || !formData.description) {
      addNotification({ type: 'product_added', title: 'Validation Error', message: 'Name and description are required' })
      return
    }
    ...
    try {
      ...
      if (editingDeal) {
        ...
        addNotification({ type: 'product_updated', title: 'Deal Updated', message: `${dealPayload.name} updated!` })
      } else {
        await dealService.add(dealPayload)
        addNotification({ type: 'product_added', title: 'Deal Added', message: `${dealPayload.name} added!` })
      }
      closeModal()
    } catch (err: any) {
      console.error('Save deal error:', err)
      addNotification({ type: 'product_added', title: 'Error', message: err?.message || 'Failed to save deal.' })
    } finally {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      addNotification({ type: 'product_deleted', title: 'Deleted', message: 'Deal removed.' })
    } catch (err: any) {
      addNotification({ type: 'product_deleted', title: 'Error', message: err?.message || 'Delete failed.' })
    }
  }
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-neutral-50">
              <tr>
                <th className="px-4 py-3 text-left text-sm font-semibold text-neutral-700">Deal</th>
                <th className="px-4 py-3 text-left text-sm font-semibold text-neutral-700">Price</th>
                <th className="px-4 py-3 text-left text-sm font-semibold text-neutral-700">Items</th>
            ...
            <tbody className="divide-y divide-neutral-100">
                ...
                <tr key={deal.id} className="hover:bg-neutral-50">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      {deal.image && <img src={deal.image} alt={deal.name} className="w-10 h-10 rounded-lg object-cover" />}
                      <div>
                        <p className="font-medium text-neutral-900">{deal.name}</p>
                        <p className="text-xs text-neutral-500 line-clamp-1">{deal.description}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-4 py-3 text-sm text-neutral-600">Rs {deal.price}</td>
                  <td className="px-4 py-3 text-sm text-neutral-600">{deal.items.length} items</td>
                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <button onClick={() => openEditModal(deal)} className="p-2 text-neutral-500 hover:text-primary hover:bg-primary/10 rounded-lg transition-colors">
                        <FiEdit2 className="w-4 h-4" />
                      </button>
                      <button onClick={() => handleDelete(deal.id)} className="p-2 text-neutral-500 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors">
              ...
              {filteredDeals.length === 0 && (
                <tr><td colSpan={4} className="px-4 py-8 text-center text-neutral-500">No deals found</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-6 border-b border-neutral-100">
              <h2 className="text-xl font-bold text-neutral-900">{editingDeal ? 'Edit Deal' : 'Add Deal'}</h2>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="sm:col-span-2">
                  <label className="block text-sm font-medium text-neutral-700 mb-1">Deal Name *</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-neutral-700 mb-1">Price (Rs) *</label>
                ...
                <div>
                  ...
                  />
                  <p className="text-xs text-neutral-400 mt-1">Paste image URL</p>
                  {formData.image && (
                    <img src={formData.image} alt="Preview" className="mt-2 w-16 h-16 rounded-lg object-cover border border-neutral-200"
                      onError={(e) => { (e.target as HTMLImageElement).style.display = 'none' }} />
                  )}
                </div>

                <div className="sm:col-span-2">
                  <label className="block text-sm font-medium text-neutral-700 mb-1">Items (one per line)</label>
